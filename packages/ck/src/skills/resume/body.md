...
- `$ARGUMENTS` にファイル名（または名前の一部）が指定されている場合: `.claude/works/handoffs/` から一致するファイルを選択して Step 3 へ進む（一致なしなら一覧を提示して選択に切り替える）
...
- `## Entry` ブロックが無い（旧形式・手書きなど）場合は、ファイル全体を読んで把握できた範囲でサマリーに使い、「handoff が標準形式でないため部分的な復元です」と明記する（形式エラーで停止しない）
