...
`$ARGUMENTS` にプロジェクト名が渡された場合はそれを前提にインタビューを始める（Phase 1 の「目的・ユーザー」で名前を聞き直さない）。
...
3条件を満たさない決定は ADR にせず、CONTEXT.md・CLAUDE.md 等の該当箇所に反映するだけでよい（記録しなかった旨の報告も不要）。
...
検出されたディレクトリ一覧をユーザーに提示し、「どのディレクトリに CLAUDE.md を作成しますか？」と確認する。ルートの `CLAUDE.md` は常に生成する（確認不要。ただし**既存の CLAUDE.md がある場合は上書きせず、「制約」の差分提案ルールに従う**）。ユーザーが選択したディレクトリのみ以降のステップで生成する。
