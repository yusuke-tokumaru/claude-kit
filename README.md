...
`ck note`/`ck todo` コマンドでCLI からも記録できます。ファイルはローカル（`.notes/`）またはグローバル（`~/.ck-notes/`）に保存されます。
...
**`/ck-note` — メモをMarkdownファイルに記録**

```
/ck-note Reactのメモ化はuseMemoよりReact.memoが有効なケースがある
```

**`/ck-todo` — TODO をMarkdownファイルに記録**
...
ck note <内容>             # メモを記録（ローカル: .notes/）
ck note <内容> --global    # メモを記録（グローバル: ~/.ck-notes/）
ck todo <内容>             # TODOを記録
ck todo <内容> --priority high  # 優先度付きTODO
